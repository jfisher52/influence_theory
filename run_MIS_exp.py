...
from src.super_quantile import reduce_superquantile
...
from src.approx_alg import arnoldi_iter, distill, compute_influence_on_loss
...
def run(config):
    ...
    tokenizer = get_tokenizer(config.model.tokenizer_name, config.model.name, config.model.tokenizer_class)
    model_original= get_model(tokenizer, config.model.name, transformers, config.model.class_name, config.model.pt, config.dropout,config.model.inner_params, config.no_grad_layers, base_dir, config.half).to(config.device)
    ...
    te_raw_inpt = torch.load(base_dir+"/data/zsre_selected_test_raw_inpt", map_location=config.device)
    te_raw_pred = torch.load(base_dir+"/data/zsre_selected_test_raw_pred", map_location=config.device)
    te_tok_pred = torch.load(base_dir+"/data/zsre_selected_test_tok_pred", map_location=config.device)

    test_data = Create_dataset(te_raw_inpt, te_tok_pred,te_raw_pred,tokenizer,config)
    ...
    start_vector = [torch.randn_like(v).cpu() for v in model_original.parameters()]
    result = arnoldi_iter(start_vector, config.method.arnoldi.n_it,config.method.arnoldi.lambda1,config.device,train_dataloader,model_original, verbose=False)
    eigvals, eigvecs = distill(result, config.method.arnoldi.top_k, verbose=False)
    ...
    for i,te in enumerate(test_dataloader):
        ...
        grad_loss_test_pt = torch.autograd.grad(loss_test_pt, model_original.parameters())
    ...
    for j, grad_te in enumerate(grad_loss_test_ls):
        for rd in removed_dataloader:
            ...
            influence =compute_influence_on_loss(grad_loss_removed_pt, grad_te, eigvals, eigvecs)
            logs.append(influence)
        logging.info("Complete one test point") 
        MIS.append(reduce_superquantile(torch.tensor(logs),superquantile_tail_fraction=1-config.alpha))
